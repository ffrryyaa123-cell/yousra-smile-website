...
const MARKER = 'PUBLIC_LANGUAGE_V4';
...
patch('src/components/ProductDetailModal.tsx', source => {
  ...
  return pairs(source, [
    ...
    ['                    <span>شاركي تجربتك ورأيك عن هذا المنتج:</span>', "                    <span>{language === 'ar' ? 'شاركي تجربتك ورأيك عن هذا المنتج:' : 'Share your experience with this product:'}</span>"],
    ['<label className="text-xs text-slate-400 block mb-1">اسمك الكريمة (اختياري):</label>', "<label className=\"text-xs text-slate-400 block mb-1\">{language === 'ar' ? 'اسمك (اختياري):' : 'Your name (optional):'}</label>"],
    ['placeholder="مثال: أم عبد الله"', "placeholder={language === 'ar' ? 'مثال: أم عبد الله' : 'Example: Sarah'}"],
    ['<label className="text-xs text-slate-400 block mb-1">تقييمك بالنجوم:</label>', "<label className=\"text-xs text-slate-400 block mb-1\">{language === 'ar' ? 'تقييمك بالنجوم:' : 'Your star rating:'}</label>"],
    ...
    ['                          س\n', "                          {language === 'ar' ? 'س' : 'S'}\n"],
    ['<span className="text-xs font-bold text-white">سارة الشمري</span>', "<span className=\"text-xs font-bold text-white\">{language === 'ar' ? 'سارة الشمري' : 'Sarah Alshammari'}</span>"],
    ['<span className="text-[10px] bg-emerald-950 text-emerald-400 px-2 py-0.5 rounded-full border border-emerald-500/30 mr-2">مشتري مؤكد ✓</span>', "<span className=\"text-[10px] bg-emerald-950 text-emerald-400 px-2 py-0.5 rounded-full border border-emerald-500/30 mr-2\">{language === 'ar' ? 'مشتري مؤكد ✓' : 'Verified Buyer ✓'}</span>"],
    ...
    ['                          م\n', "                          {language === 'ar' ? 'م' : 'M'}\n"],
    ['<span className="text-xs font-bold text-white">م. محمد علي</span>', "<span className=\"text-xs font-bold text-white\">{language === 'ar' ? 'م. محمد علي' : 'M. Mohammad Ali'}</span>"],
    ...
    ["{copiedJsonLd ? 'تم نسخ الـ Schema' : 'نسخ كود JSON-LD'}", "{copiedJsonLd ? (language === 'ar' ? 'تم نسخ الـ Schema' : 'Schema copied') : (language === 'ar' ? 'نسخ كود JSON-LD' : 'Copy JSON-LD')}"],
    ['"name": product.titleAr,', '"name": displayTitle,'],
    ['"description": product.description,', '"description": language === \'en\' ? (product.descriptionEn || product.longDescriptionEn || product.titleEn || \'Product review\') : (product.description || product.longDescription || product.titleAr),'],
    ['"name": "Yousra Smile (يسرى سمايل)"', '"name": language === \'en\' ? "Yousra Smile" : "Yousra Smile | يسرى سمايل"'],
...
patch('src/components/ProductCard.tsx', source => {
  source = source.replace("const displayTitle = language === 'en' ? (product.titleEn || product.titleAr) : product.titleAr;", "const displayTitle = language === 'en' ? (product.titleEn || product.brand || 'Product') : (product.titleAr || product.titleEn || product.brand || 'منتج');");
  source = source.replace("const displayDesc = language === 'en' ? (product.descriptionEn || product.description) : product.description;", "const displayDesc = language === 'en' ? (product.descriptionEn || '') : product.description;");
  source = source.replace("'التقييم غير متاح'", "(language === 'ar' ? 'التقييم غير متاح' : 'Rating unavailable')");
  return source;
});
...
console.log('[patch-public-language-v4] Safe public bilingual sweep applied.');
