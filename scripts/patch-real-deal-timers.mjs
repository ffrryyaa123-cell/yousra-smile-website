...
// Never present simulated countdowns as if they were merchant-provided deal deadlines.
// A countdown may be restored only when a real source-provided end timestamp exists.
const adminFile = new URL('../src/pages/AdminPage.tsx', import.meta.url);
let admin = fs.readFileSync(adminFile, 'utf8');
admin = admin.replace(
  /<span>ينتهي العرض بعد:\s*18 ساعة<\/span>/g,
  '<span>مدة العرض: حسب المصدر</span>'
);
fs.writeFileSync(adminFile, admin, 'utf8');

const tickerFile = new URL('../src/components/FlashDealsTicker.tsx', import.meta.url);
const ticker = `import React from 'react';
...
`;
fs.writeFileSync(tickerFile, ticker, 'utf8');

console.log('[patch-real-deal-timers] Removed simulated countdowns; only source-backed expiry should be shown.');
