...
export const SmartSavingsCalculator: React.FC = () => {
  const { formatPrice, setPage, setSelectedCategory, openProductDetail, products, language } = useApp();
      ...
      <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-purple-500/20 pb-5">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/20 text-amber-300 text-xs font-black border border-amber-500/30 mb-2">
            ...
            <span>{language === 'ar' ? 'حاسبة ذكية تفاعلية • التوفير المالي والزمني' : 'Smart Savings Calculator'}</span>
          </div>
        ...
        <div className="bg-slate-900/90 border border-purple-500/40 rounded-2xl p-3 shrink-0 flex items-center gap-3">
          <span className="text-xs text-slate-300 font-bold">{language === 'ar' ? 'حجم العائلة:' : 'Family size:'}</span>
        ...
        <div className="lg:col-span-5 bg-gradient-to-br from-slate-950 to-[#190d2e] border border-amber-500/40 rounded-2xl p-5 flex flex-col justify-between shadow-xl">
          <div>
            <div className="flex items-center justify-between border-b border-purple-500/20 pb-3 mb-4">
              <span className="text-xs font-bold text-slate-300">{language === 'ar' ? 'النتيجة التقديرية المباشرة:' : 'Estimated savings:'}</span>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3.5 text-center">
                <span className="text-[11px] text-slate-400 block mb-1">{language === 'ar' ? 'الوقت الموفّر شهرياً' : 'Monthly time saved'}</span>
                <div className="text-2xl sm:text-3xl font-black text-amber-400 font-['Tajawal'] flex items-center justify-center gap-1">
                  ...
                  <span className="text-xs font-normal text-slate-300">{language === 'ar' ? 'ساعة' : 'hours'}</span>
                </div>
              ...
              <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3.5 text-center">
                <span className="text-[11px] text-slate-400 block mb-1">{language === 'ar' ? 'التوفير المالي السنوي' : 'Estimated annual savings'}</span>
            ...
            <div className="space-y-2 mb-4">
              <span className="text-[11px] font-bold text-slate-400 block">{language === 'ar' ? 'منتجات مقترحة:' : 'Suggested products:'}</span>
                ...
                >
                  <div className="flex items-center gap-2 overflow-hidden">
                    ...
                    <div className="truncate">
                      ...
                      <div className="text-[10px] text-emerald-400">{language === 'ar' ? 'تحقق من العرض الحالي' : 'Check current deal'}</div>
                    </div>
                  </div>
          ...
          >
            ...
            <span>{language === 'ar' ? 'استعرض جميع المنتجات الذكية' : 'Browse Smart Products'}</span>
