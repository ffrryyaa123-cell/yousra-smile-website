...
export const VideoModal: React.FC<VideoModalProps> = ({ video, onClose }) => {
  ...
  const displayProductTitle = language === 'en'
    ? (linkedProduct?.titleEn || linkedProduct?.brand || 'Product')
    : (linkedProduct?.titleAr || linkedProduct?.titleEn || video.productTitle || 'منتج');
  const displayVideoTitle = language === 'en'
    ? (linkedProduct?.titleEn
      ? `Yousra Smile Review: ${linkedProduct.titleEn}`
          ...
          <div className="aspect-video w-full bg-black relative">
            {isDirectOrLocal ? (
              <video 
                ...
                src={video.videoUrl}
                    poster={video.hideThumbnail ? undefined : video.thumbnailUrl || video.productImage}
                controls
                autoPlay
                playsInline
              />
