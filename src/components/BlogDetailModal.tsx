...
import { X, Calendar, Clock, User, Share2, ShoppingBag, ExternalLink } from 'lucide-react';
...
export const BlogDetailModal: React.FC<BlogDetailModalProps> = ({ post, onClose }) => {
  ...
  const postTitle = language === 'ar' ? post.titleAr : post.titleEn;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-200 font-['Cairo']">
        ...
        <div className="relative h-48 sm:h-64 overflow-hidden shrink-0">
          ...
          <div className="absolute bottom-4 right-4 left-4">
            <span className="bg-amber-400 text-slate-950 text-xs font-black px-3 py-1 rounded-full inline-block mb-2">
              {language === 'ar' ? post.category : (post.categoryEn || 'Buying Guide')}
            </span>
            <h2 className="text-xl sm:text-2xl font-black text-white leading-tight">{postTitle}</h2>
            <div className="flex items-center gap-4 text-xs text-slate-300 mt-2 flex-wrap">
              <span className="flex items-center gap-1">
                <User className="w-3.5 h-3.5 text-amber-400" />
                <span>{language === 'ar' ? post.authorName : (post.authorNameEn || 'Yousra Smile')}</span>
              </span>
              <span className="flex items-center gap-1">
                <Clock className="w-3.5 h-3.5 text-amber-400" />
                <span>{language === 'ar' ? post.readTime : (post.readTimeEn || 'Quick read')}</span>
              </span>
              <span className="flex items-center gap-1">
          ...
          {relatedProducts.length > 0 && (
            <div className="mt-8 pt-6 border-t border-slate-800">
              <h3 className="text-lg font-bold text-amber-300 mb-4 flex items-center gap-2">
                ...
                <span>{language === 'ar' ? 'المنتجات المذكورة في هذا الدليل:' : 'Products Mentioned in This Guide:'}</span>
              </h3>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {relatedProducts.map(product => {
                  const productTitle = language === 'ar'
                    ? (product.titleAr || product.titleEn || product.brand)
                    : (product.titleEn || product.brand || 'Product');
                  return (
                    <div
                      key={product.id}
                    ...
                    >
                      <img src={product.image} alt={productTitle} className="w-20 h-20 object-cover rounded-xl shrink-0" />
                      <div className="flex-1 min-w-0">
                        <h4 className="text-xs font-bold text-white line-clamp-2">{productTitle}</h4>
                        ...
                        <button className="mt-2 text-[11px] bg-amber-400 text-slate-950 font-extrabold px-3 py-1 rounded-lg flex items-center gap-1">
                          <span>{language === 'ar' ? 'تفاصيل المنتج والشراء' : 'Product Details & Buy'}</span>
