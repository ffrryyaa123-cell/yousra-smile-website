      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] bg-slate-900 border border-amber-500/30 rounded-3xl shadow-2xl overflow-hidden flex flex-col text-white">
            ...
            <div className="mt-8 pt-6 border-t border-slate-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {relatedProducts.map(product => (
                  ...
                  >
                    <img src={product.image} alt={language === 'en' ? (product.titleEn || product.brand) : product.titleAr} className="w-20 h-20 object-cover rounded-xl shrink-0" />
                    <div className="flex-1 min-w-0">
                      <h4 className="text-xs font-bold text-white line-clamp-2">{language === 'en' ? (product.titleEn || product.brand) : product.titleAr}</h4>
                      ...
                      <button className="mt-2 text-[11px] bg-amber-400 text-slate-950 font-extrabold px-3 py-1 rounded-lg flex items-center gap-1">
                        <span>{language === 'ar' ? 'تفاصيل المنتج والشراء' : 'Product Details & Purchase'}</span>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950 flex items-center justify-between">
          <button
            onClick={() => {
              if (navigator.share) {
                navigator.share({ title: language === 'ar' ? post.titleAr : post.titleEn, url: window.location.href });
              } else {
