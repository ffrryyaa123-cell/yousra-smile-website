      ...
      {comparedProducts.length > 0 ? (
          ...
          <table className="w-full text-right text-xs">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/60 divide-x divide-x-reverse divide-slate-200 dark:divide-slate-800">
                <th className="p-4 text-slate-400 font-bold w-48">{language === 'en' ? 'Comparison' : 'خاصية المقارنة'}</th>
                {comparedProducts.map(prod => (
                  <th key={prod.id} className="p-4 w-72 min-w-[240px] align-top">
                    <div className="relative space-y-2">
                      ...
                      <img 
                        src={prod.image} 
                        alt={language === 'en' ? (prod.titleEn || 'Product') : prod.titleAr}
                      ...
                      <h3 className="text-xs font-bold text-slate-900 dark:text-white line-clamp-2 font-['Tajawal']">
                        {language === 'en' ? (prod.titleEn || 'Featured product') : prod.titleAr}
                      </h3>
                    </div>
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              <tr className="divide-x divide-x-reverse divide-slate-100 dark:divide-slate-800">
                <td className="p-4 font-bold text-slate-900 dark:text-white bg-slate-50/50 dark:bg-slate-800/30">{language === 'en' ? 'Price & Savings' : 'السعر والتوفير'}</td>
                {comparedProducts.map(prod => (
              ...
              {/* Row 2: Rating */}
              <tr className="divide-x divide-x-reverse divide-slate-100 dark:divide-slate-800">
                <td className="p-4 font-bold text-slate-900 dark:text-white bg-slate-50/50 dark:bg-slate-800/30">{language === 'en' ? 'Rating & Demand' : 'التقييم وسرعة الطلب'}</td>
                {comparedProducts.map(prod => (
                  <td key={prod.id} className="p-4">
                    <div className="flex items-center gap-1 text-amber-500 font-bold">
                      <Star className="w-4 h-4 fill-amber-400" />
                      <span>{prod.rating}</span>
                      <span className="text-slate-400 font-normal">({prod.reviewCount} {language === 'en' ? 'reviews' : 'تقييم'})</span>
                    </div>
                  </td>
                ))}
              </tr>
              ...
              {/* Row 3: Category */}
              <tr className="divide-x divide-x-reverse divide-slate-100 dark:divide-slate-800">
                <td className="p-4 font-bold text-slate-900 dark:text-white bg-slate-50/50 dark:bg-slate-800/30">{language === 'en' ? 'Category & Type' : 'القسم والتخصص'}</td>
                {comparedProducts.map(prod => (
              ...
              {/* Row 4: Key Features */}
              <tr className="divide-x divide-x-reverse divide-slate-100 dark:divide-slate-800">
                <td className="p-4 font-bold text-slate-900 dark:text-white bg-slate-50/50 dark:bg-slate-800/30">{language === 'en' ? 'Key Features' : 'أبرز المميزات'}</td>
                {comparedProducts.map(prod => (
              ...
              <tr className="divide-x divide-x-reverse divide-slate-100 dark:divide-slate-800 bg-slate-50/80 dark:bg-slate-800/50">
                <td className="p-4 font-bold text-slate-900 dark:text-white">{language === 'en' ? 'Direct Purchase' : 'الانتقال للشراء المباشر'}</td>
                {comparedProducts.map(prod => (
                  <td key={prod.id} className="p-4">
                    <button
                      onClick={() => {
        ...
        <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-8 space-y-4">
          ...
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 font-['Tajawal']">
            {language === 'en' ? 'You have not added products to compare yet.' : 'لم تقومي بإضافة أي منتجات للمقارنة بعد!'}
          </h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            {language === 'en' ? 'Select the scale icon on a product card to add it here.' : 'انقري على رمز الميزان في بطاقات المنتجات لإضافتها لمصفوفة المقارنة المباشرة.'}
          </p>
          <button
