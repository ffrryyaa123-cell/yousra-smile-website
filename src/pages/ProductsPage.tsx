  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      ...
      if (filters.searchQuery) {
        ...
        const keywordMatch = product.keywords && product.keywords.some(k => String(k ?? '').toLowerCase().includes(query));
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto flex-wrap">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-400 shrink-0">{language === 'ar' ? 'الترتيب:' : 'Sort:'}</span>
                <select
                ...
                >
                  <option value="latest">{language === 'ar' ? 'الأحدث نزوَلاً' : 'Latest'}</option>
                  <option value="rating">{language === 'ar' ? 'الأعلى تقييمًا (★ 5.0)' : 'Highest Rated'}</option>
                  <option value="best-selling">{language === 'ar' ? 'الأكثر مبيعًا وطلباً' : 'Best Selling'}</option>
                  <option value="highest-discount">{language === 'ar' ? 'الأعلى نسبة خصم' : 'Highest Discount'}</option>
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-400 shrink-0">{language === 'ar' ? 'عرض بالصفحة:' : 'Per page:'}</span>
                <select
                  value={itemsPerPage}
                  onChange={(e) => {
                  ...
                  className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-1.5 text-xs text-white focus:outline-none focus:border-purple-500 font-semibold"
                >
                  <option value={12}>{language === 'ar' ? '12 منتج' : '12 products'}</option>
                  <option value={20}>{language === 'ar' ? '20 منتج (الكل)' : '20 products'}</option>
                  <option value={50}>{language === 'ar' ? '50 منتج' : '50 products'}</option>
                </select>
              </div>
            </div>
