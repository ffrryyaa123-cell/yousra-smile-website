...
import { supabaseCatalog } from '../services/supabaseCatalog';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Supabase is the catalogue we are moving to. It only overwrites what is on
    // screen once it actually returns products, so an empty or unreachable
    // Supabase leaves the Firestore catalogue exactly as it is.
    const stopSupabaseProducts = supabaseCatalog.subscribeProducts(remoteProducts => {
      setProducts(remoteProducts.map(normalizeProduct));
    });
    const stopSupabaseVideos = supabaseCatalog.subscribeVideos(remoteVideos => {
      setVideos(remoteVideos);
    });

    return () => { stopProducts(); stopVideos(); stopSupabaseProducts(); stopSupabaseVideos(); };
  }, []);
