  ...
  app.post("/api/agent/auto-curate", verifyAgentAuth, async (req, res) => {
    try {
      ...
      if (aiClient) {
        try {
          const prompt = `
          أنت وكيل ذكاء اصطناعي خبير (AI E-commerce Curator & Affiliate Specialist) لمنصة "يسرى سمايل" (yousrasmile.com) المتخصصة في مراجعات الأجهزة الذكية والمنزل العصري.
  ...
  app.post("/api/agent/batch-curate", verifyAgentAuth, async (req, res) => {
    try {
      ...
      if (aiClient) {
        try {
          const prompt = `
          أنت وكيل الذكاء الاصطناعي الرئيسي (Lead AI Bulk E-commerce Agent & Affiliate Specialist) لمنصة "يسرى سمايل" (yousrasmile.com).
